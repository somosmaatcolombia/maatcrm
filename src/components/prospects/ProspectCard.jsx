...
export default function ProspectCard({ prospect, onView, onEdit, onDelete, showAdvisor = false }) {
  ...
  return (
    <div className={`bg-white rounded-xl shadow-md p-5 transition-shadow duration-200 hover:shadow-lg group ${
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="p-1 text-gray-400 hover:text-[#333333] rounded-md hover:bg-gray-100 transition-colors duration-200 opacity-0 group-hover:opacity-100"
          >
          ...
          {menuOpen && (
            <div className="absolute right-0 top-8 bg-white rounded-lg shadow-lg border border-gray-100 py-1 w-40 z-20">
              ...
              <button
                onClick={() => { onDelete?.(prospect); setMenuOpen(false) }}
      ...
      <div className="space-y-1.5 text-xs text-[#6B7280]">
        ...
        {prospect.next_contact_date && (
          <div className={`flex items-center gap-2 ${contactOverdue ? 'text-red-600 font-medium' : ''}`}>
            ...
            <span>{contactOverdue ? 'Vencido: ' : 'Próximo contacto: '}{formatDate(prospect.next_contact_date)}</span>
          </div>
        )}
      </div>
